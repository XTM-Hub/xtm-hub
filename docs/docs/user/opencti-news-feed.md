...
News Feed notifies your connected OpenCTI product whenever new content is published on the XTM Hub (for example a new custom dashboard, playbook, or custom view), so you can quickly review it and decide whether to import it, without having to check the Hub manually.

## Requirements

To receive News Feed notifications in OpenCTI, you need:

- An OpenCTI product **connected to the XTM Hub** (see [OpenCTI Product Connection](opencti-connection.md)).
...
As soon as OpenCTI retrieves a new News Feed item, it shows a toast notification so you immediately know new content is available.
