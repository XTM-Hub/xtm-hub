...
interface MultiSelectFormFieldProps<
  // React.forwardRef can't preserve a generic type parameter, so T collapses
  // to this default for every caller regardless of the concrete option type
  // passed in; `any` (rather than `unknown`) is required so options arrays of
  // plain interfaces (without an index signature) remain assignable.
                  ...
                  {options.map((option) => {
                    ...
                    return (
                      <CommandItem
                        ...
                        {isSelected ? (
                          <div className="mr-2 flex h-4 w-4 min-w-4 items-center justify-center rounded-xs border border-primary bg-primary text-primary-foreground">
                            <CheckIcon className="h-4 w-4" />
                          </div>
                        ) : (
                          <div className="mr-2 flex h-4 w-4 min-w-4 items-center justify-center rounded-xs border border-primary opacity-50"></div>
                        )}
